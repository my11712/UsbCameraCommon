...
//                            saveYuv2Jpeg(picPath, yuv);
        ...
        private void saveYuv2Jpeg(String path, byte[] data) {
            ...
            boolean result = yuvImage.compressToJpeg(new Rect(0, 0, 320, 240), 85, bos);
            if (result) {
        ...
        private void saveJpeg(String path, Integer width, Integer height, byte[] data) {
            Activity activity = mWeakParent.get();
            if (activity != null) {
                NV21ToBitmap nv21ToBitmap = new NV21ToBitmap(activity);
                Bitmap bitmap = nv21ToBitmap.nv21ToBitmap(data, mWidth, mHeight);
                if (mWidth != width || mHeight != height) {
                    bitmap = scaleWithWH(bitmap, width, height);
                }
                if (!TextUtils.isEmpty(path)) {
                    if (saveBitmap(path, bitmap)) {
                        if (mCaptureListener != null) mCaptureListener.onCaptureResult(path);
                    } else {
                        isCaptureStill = true;
                    }
                } else {
        ...
        public boolean saveBitmap(String path, Bitmap bitmap) {
            boolean result;
            File file = new File(path);
